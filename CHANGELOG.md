...
### Added
- **Configurable fallback-commit hook behavior via `worktreeCommitNoVerify`.** Worktree cleanup previously hardcoded `git commit --no-verify`, preventing repositories from enforcing pre-commit or commit-msg policy when an isolated agent left uncommitted changes. The default remains `true` for compatibility; set it to `false` in `subagents.json` or disable `/agents → Settings → Skip worktree hooks` to run hooks. A rejected hook now keeps the worktree and reports its path instead of deleting unpreserved changes.
- **Configurable worktree prefixes via `worktreeBranchPrefix`.** Isolated agents previously always used `pi-agent` for both temporary worktree directories and branches, which can conflict with repository naming policy. Set a project or global value in `subagents.json`, or use `/agents → Settings → Worktree prefix`, to produce `<prefix>-<agentId>-<suffix>` directories and `<prefix>-<agentId>` branches for worktrees created after the change. The default remains `pi-agent`. Values are limited to a conservative Git-safe subset (letters, numbers, `.`, `_`, and `-`, up to 100 characters, starting alphanumeric, without `..`) so an invalid hand-edited prefix cannot make cleanup fail while preserving agent work.
...
### Fixed
- **`isolation: "worktree"` agents are now told to stay in the worktree** ([cad0p/pi-subagents-tintinweb#10](https://github.com/cad0p/pi-subagents-tintinweb/pull/10) — thanks [@cad0p](https://github.com/cad0p)). Append-mode agents inherit the parent's system prompt, which names the shared checkout as the working directory; agents could therefore leave their isolated copy and race on the main tree's HEAD. Worktree spawns now receive a `<worktree_isolation>` directive naming both paths and requiring all work to stay in the copy. The directive is present in append and replace prompt modes; command-level enforcement remains out of scope.
