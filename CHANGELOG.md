...
- **Configurable worktree branch prefixes via `worktreeBranchPrefix`.** Isolated agents previously always preserved changes on `pi-agent-<agentId>`, which can conflict with repository naming policy. Set a project or global value in `subagents.json`, or use `/agents → Settings → Worktree branch prefix`, to produce `<prefix>-<agentId>` branches for worktrees created after the change. The default remains `pi-agent`; temporary worktree directory names are unchanged. Values are limited to a conservative Git-safe subset (letters, numbers, `.`, `_`, and `-`, up to 100 characters, starting alphanumeric, without `..`) so an invalid hand-edited prefix cannot make cleanup fail while preserving agent work.
